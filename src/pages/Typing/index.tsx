...
import { currentChapterAtom, currentDictIdAtom, dynamicDictMapAtom, isReviewModeAtom, randomConfigAtom, reviewModeInfoAtom } from '@/store'
...
const App: React.FC = () => {
  ...
  const dynamicDictMap = useAtomValue(dynamicDictMapAtom)
  ...
  useEffect(() => {
    ...
    // Check both static dictionary map and dynamic dictionary map (for error book collection)
    if (!(id in idDictionaryMap) && !(id in dynamicDictMap)) {
      setCurrentDictId('ieltsWang3')
      setCurrentChapter(0)
      return
    }
  }, [currentDictId, setCurrentChapter, setCurrentDictId, dynamicDictMap])
